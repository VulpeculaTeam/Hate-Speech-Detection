...
class SentimentAnalysis(object):
    ...
    def __init__(self, filename='polarita.txt', weighting='geometric'):
    ...
    def build_swn(self, filename, weighting):
        ...
        records = [line.split('\t') for line in open(filename, encoding="utf8")]
        for rec in records:
...
def calcolaPolarita(frase):
    s = SentimentAnalysis(filename='SentiWordNetItalian.txt',weighting='geometric')

    return s.score(frase)
'''
import csv

filepath = 'SentiWordNetItalian2.txt'  
...
gs = goslate.Goslate(service_urls=['http://translate.google.it'])
parole2 = []
parole4 = []

miofile = open("testfile.txt","w") 
lista = []
with open(filepath) as fd:
    ...
    for row in rd:
        time.sleep(1)
        parole = row[4].split()
        for i in range(0, len(parole)):
            parole2 = parole[i].split("#")
            #parole2[0] = gs.translate(parole2[0],'it')
            try:
                traduzione = translator.translate(parole2[0], dest='it')
            except ValueError:
                break
            parole2 = traduzione.text + "#"+parole2[1]
            #print(parole2)
        try:
            traduzione2 = translator.translate(row[5], dest='it')
        except ValueError:
                break
        #row[5] =  gs.translate(row[5],'it')
        row[5] = traduzione2.text
        row[4] = parole2
        lista.append(row)
        print(row)
#with open("nuovoFile.txt","w",newline="",encoding="utf8") as f:  
#    cw = csv.writer(f)
#    cw.writerows(r+["\t"] for r in lista)

with open("nuovoFile2.txt","w",encoding="utf8",newline='') as fd:
    rd = csv.writer(fd, delimiter="\t", quotechar='|')
    for r in lista:
        print(r[5])
    rd.writerows(r for r in lista)
'''